...
const scrapeCreatorsAdapter =
  ...
  async (handle) => {
    ...
    if (response.status === 401 || response.status === 402 || response.status === 403) {
      console.error(`[beacon/provider] ScrapeCreators auth/credits issue (HTTP ${response.status}) — check the API key and remaining credits`);
      throw new ProviderHttpError("unavailable", `Vendor auth/credits issue (${response.status})`);
    }
    if (!response.ok) {
      console.error(`[beacon/provider] ScrapeCreators returned HTTP ${response.status}`);
      throw new ProviderHttpError("unavailable", `Vendor returned ${response.status}`);
    }
    const payload = await safeJson(response);
    const user = (payload as { data?: { user?: RawUser | null } })?.data?.user;
    if (!user) {
      console.error("[beacon/provider] ScrapeCreators response missing data.user — unexpected shape");
      throw new ProviderHttpError("not_found");
    }
    return user;
  };
...
function resolveAdapter(): VendorAdapter {
  ...
  // Running on Vercel (or any datacenter): the keyless direct endpoint is
  // login-walled from datacenter IPs (KNOWN_ISSUES §A.4), so a missing key
  // means EVERY real profile silently fails as "unavailable". Make that loud.
  const onServerless = Boolean(process.env.VERCEL ?? process.env.VERCEL_ENV);
  if (key) {
    console.info("[beacon/provider] using ScrapeCreators adapter");
    return scrapeCreatorsAdapter(key);
  }
  if (onServerless) {
    // Do not silently fall back to an endpoint we KNOW is blocked here — that
    // is the exact failure that made Beacon look broken for real profiles.
    console.error(
      "[beacon/provider] SCRAPECREATORS_API_KEY is not set in this deployment. " +
        "The keyless Instagram endpoint is login-walled from datacenter IPs, so " +
        "real profiles cannot be analyzed. Set the env var in Vercel and redeploy.",
    );
    throw new ProviderHttpError(
      "unavailable",
      "Provider not configured: SCRAPECREATORS_API_KEY missing",
    );
  }
  console.warn("[beacon/provider] no API key — using keyless direct endpoint (local dev only)");
